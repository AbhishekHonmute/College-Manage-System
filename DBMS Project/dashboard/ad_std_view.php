...
<body>
    <?php 
        ...
        $read_data = 1;
        if ($read_data == 1) {
            $sql = "SELECT fname, lname, total_credits, mobile, email, city, dob, joining_year, dept_id FROM student WHERE mis = ".$mis." ";
        ...
        $total_courses = "0";
        $sql = "SELECT course_id from course where course_id in (select course_id from takes where mis = ".$mis.")";
        if ($result = $conn->query($sql)) {
        ...
        <div class="page-wrapper">
            <div class="container-fluid">
                                ...
                                <form class="form-horizontal form-material" method="POST" ecntype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" autocomplete="off">
                                    ...
                                    <div class="form-group mb-4">
                                        ...
                                        <div class="col-md-12 border-bottom p-0">
                                            <input type="number" value="<?php echo $mis ?>" class="form-control p-0 border-0" name="mis" disabled> </div>
                                    </div>
                                    ...
                                    <div class="form-group mb-4">
                                        <label class="col-md-12 p-0">Total total_credits</label>
                                        <div class="col-md-12 border-bottom p-0">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="white-box">
                            <h3 class="box-title">Enrolled in Courses : </h3>
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php 
                                            $sql = "SELECT course_id, title, i_mis, credits, dept_id from course where course_id in (select course_id from takes where mis = ".$mis.");";
                                            if ($result = $conn->query($sql)) {
                                                if ($result->num_rows >= 1) {
    ...
    <?php 
        if($done == 5) {
            $sql = "UPDATE student SET fname = '".$fname."', lname = '".$lname."', total_credits = ".$total_credits.", mobile = ".$mobile.", email = '".$email."', city = '".$city."', dob = '".$dob."', joining_year = ".$joining_year.", dept_id = ".$dept_id." WHERE mis = ".$mis." ";
        ...
        if (isset($_POST['submit'])) {
            if ($_POST['submit'] == "remove") {
                $sql = "DELETE FROM student WHERE mis = '".$mis."'";
                if ($result = $conn->query($sql)) {
                    echo "<script>alert('Student Deleted Successfully!!');</script>";
                    echo "<script>window.location.replace('ad_student.php');</script>";
                } else {
