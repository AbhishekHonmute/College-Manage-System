...
<body>
                        ...
                        <div class="white-box">
                            <div class="row align-items-center">
                                <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                                    <h2 class="page-title text-uppercase font-medium font-20">Department</h2>
                                </div>
                                <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                                    <div class="d-md-flex">
                                        <ol class="breadcrumb ml-auto">
                                        </ol>
                                        <button onclick="add_dept_handler()" target="_blank" class="btn btn-success  d-none d-md-block pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light">Add Department</button>
                                    </div>
                                </div>
                            </div>
                            <div class="table-responsive">
                                <table class="table">
                                    <thead>
                                        <tr>
                                            <th class="border-top-0">Deptartment Id</th>
                                            <th class="border-top-0">Deptartment Name</th>
                                            <th class="border-top-0">Budget</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php 
                                            $sql = "SELECT dept_id, dept_name, budget FROM department";
                                            if ($result = $conn->query($sql)) {
                                                if ($result->num_rows >= 1) {
                                                    // output data of each row
                                                    while($row = $result->fetch_assoc()) {
                                                        echo "<tr><td>" . $row["dept_id"]. "</td><td>" . $row["dept_name"]. "</td><td>" . $row["budget"]. "</td><td><button onclick='view(\"ad_dept_view_id\", " . $row["dept_id"]. ", \"ad_dept_view.php\")' class='btn btn-block btn-danger text-white'>View</button></td></tr>";
                                                    }
                                                }
                                            } else {
                                                echo "Query Failed";
    ...
    <script> 
        ...
        function add_dept_handler() {
            window.location.replace("ad_add_dept.php");
        }
    </script>
</body>
