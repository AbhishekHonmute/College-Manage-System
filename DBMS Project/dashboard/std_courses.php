...
<body>
                            ...
                            <div class="row align-items-center">
                                    ...
                                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                                        <div class="d-md-flex">
                                            ...
                                            <button onclick="generate_marklist_handler()" target="_blank" class="btn btn-success  d-none d-md-block pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light">Generate Marklist</button>
                                        </div>
                                    </div>
                                </div>
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php 
                                            require "connection.php";
                                            $sql = "SELECT course_id, title,  credits, dept_id, i_mis FROM course WHERE course_id IN (SELECT course_id FROM takes WHERE mis = '".$mis."')";
                                            if ($result = $conn->query($sql)) {
                                                if ($result->num_rows >= 1) {
                                                    // output data of each row
                                                    while($row = $result->fetch_assoc()) {
                                                        echo "<tr><td>" . $row["course_id"]. "</td><td>" . $row["title"]. "</td><td>" . $row["credits"]. "</td><td>" . $row["dept_id"]. "</td><td>" . $row["i_mis"]. "</td><td><button onclick='view(\"std_course_view_id\", " . $row["course_id"]. ", \"std_course_view.php\")' class='btn btn-block btn-danger text-white'>View</button></td></tr>";
                                                    }
                                                }
                                            } else {
                                                echo "Query Failed";
    ...
    <script> 
        ...
        function generate_marklist_handler() {
            window.open("http://localhost:8080/dbms_project/std_report.php");
        }
    </script>
</body>
