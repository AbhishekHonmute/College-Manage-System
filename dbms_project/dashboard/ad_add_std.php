...
<body>
    <?php 
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (empty($_POST['lname'])) {
                $lname_err = "fnalnameme is required";
            } else {
            ...
            if (empty($_POST['joining_year'])) {
                $joining_year_err = "Joining Year is required";
            } else {
            ...
            if (empty($_POST['total_credits'])) {
                $total_credits_err = "MIS is required";
            } else {
                $total_credits = test_input($_POST['total_credits']);
                $total_credits_err = "";
                $done = $done + 1;
            }
        ...
        <div class="page-wrapper">
            <div class="container-fluid">
                ...
                <div class="col-lg-12 col-xlg-10 col-md-12">
                        <div class="card">
                            <div class="card-body">
                                <h3 class="box-title">ADD STUDENT : </h3><br>
                                <form class="form-horizontal form-material" method="POST" enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" autocomplete="off">
                                    <p id="mis_err" style="color:red"><?php echo $mis_err;?></p>
                                    <div class="form-group mb-4">
                                        <label class="col-md-12 p-0">MIS</label>
                                        <div class="col-md-12 border-bottom p-0">
                                            <input type="number" value="<?php echo $mis ?>" class="form-control p-0 border-0" name="mis"> </div>
                                    </div>
                                    ...
                                    <p id="total_credits_err" style="color:red"><?php echo $total_credits_err;?></p>
                                    <div class="form-group mb-4">
                                        <label class="col-md-12 p-0">Total Credits</label>
                                        <div class="col-md-12 border-bottom p-0">
                                            <input type="number" value="<?php echo $total_credits ?>" class="form-control p-0 border-0" name="total_credits"  > </div>
                                    </div>
                                    ...
                                    <div class="form-group mb-4">
                                        <label class="col-md-12 p-0">Adviser</label>
                                        <div class="col-md-12 border-bottom p-0">
                                            <select class="form-control p-0 border-0" name="adviser">
                                                <?php
                                                    $sql = "SELECT i_mis, fname, lname, dept_id FROM instructor";
                                                    if ($result = $conn->query($sql)) {
                                                        if ($result->num_rows >= 1) {
                                                            while($row = $result->fetch_assoc()) {
                                                                echo "<option value='".$row['i_mis']."'>mis: " . $row["i_mis"]. ", name : ".$row['fname']." ".$row['lname'].", dept : ".$row['dept_id']."</option>";
                                                            }
                                                        }
                                                    } else {
                                                        echo "Query Failed";
    ...
    <?php 
        if($done == 11) {
            $sql = "SELECT mis FROM student WHERE mis = ".$mis." ";
            if ($result = $conn->query($sql)) {
                if ($result->num_rows == 0) {
                    $target_file =  "student_photos/" . basename($_FILES["photo"]["name"]);
                    $sql = "INSERT INTO student VALUES (".$mis.", '".$fname."', '".$lname."', ".$mobile.", '".$email."', '".$city."', '".$dob."', ".$joining_year.", ".$total_credits.", ".$dept_id.", '".$target_file."')";
                    if ($result = mysqli_query($conn, $sql)) {
                        $_SESSION['ad_std_view_id'] = $mis;
                        if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
                            echo "The file has been uploaded.";
                        } else {
                            echo "Sorry, there was an error uploading your file.";
                        }
                        $sql = "INSERT INTO advisor VALUES(".$mis.", ".$adviser.")";
                        if ($result = mysqli_query($conn, $sql)) {
                        } else {
                            echo "query failed".$conn->error;
                        }
                        echo "<script>window.location.replace('ad_std_view.php');</script>";
                    } else {
                        echo "query failed".$conn->error;
                    }
                } else {
                    echo "<script>alert('MIS of student already exists. Please choose unique one.')</script>";
                }
            } else {
                echo "Query Failed";
            }
